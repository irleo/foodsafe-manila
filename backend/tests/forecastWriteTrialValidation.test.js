...
test("accepts complete forecasts across a December-to-January boundary", () => {
  assert.doesNotThrow(() => validateForecastWriteTrial(fixture(), datasetId, diseases, 10));
});

test("rejects a valid-looking forecast for a different database dataset", () => {
  assert.throws(() => validateForecastWriteTrial(fixture(), "b12345678901234567890123a", diseases, 10));
});
...
test("rejects partial models even when the top-level run claims success", () => {
  ...
  assert.throws(() => validateForecastWriteTrial(data, datasetId, diseases, 10));
});
...
test("rejects stale targets, schema versions, and non-finite counts", () => {
  ...
  assert.throws(() => validateForecastWriteTrial(stale, datasetId, diseases, 10));
  assert.throws(() => validateForecastWriteTrial(fixture(), datasetId, diseases, 11));
  const invalid = fixture();
  invalid.payload.diseases[0].districts[0].models.prophet.forecast[0].predictedCases = NaN;
  assert.throws(() => validateForecastWriteTrial(invalid, datasetId, diseases, 10));
});

test("rejects missing diseases and mismatched payload basis", () => {
  assert.throws(() => validateForecastWriteTrial(fixture(), datasetId, [...diseases, "Missing"], 10));
  const data = fixture();
