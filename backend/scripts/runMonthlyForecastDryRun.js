...
async function main() {
  ...
  if (!uri) throw new Error("Set TEST_MONGO_URI to a read-only testing database connection.");
  ...
  if (process.env.FORECAST_DRY_RUN !== "true") {
    throw new Error("This entry point requires FORECAST_DRY_RUN=true and never supports writes.");
  }
  ...
  const cancel = () => controller.abort(new Error("Forecast dry run cancelled."));
  const timer = setTimeout(() => controller.abort(new Error("Forecast dry run exceeded 30 minutes.")), 30 * 60_000);
  ...
  try {
    ...
    const { refreshMonthlyDistrictPredictions } = await import("../services/predictions/refreshMonthlyDistrictPredictions.js");
    ...
    const models = diseases.flatMap((disease) => disease.districts.map((district) => ({
    ...
    const summary = {
      ...
      successfulModels: models.filter((model) => model.status === "success").length,
      failedModels: models.filter((model) => model.status === "failed").length,
      insufficientModels: models.filter((model) => model.status === "insufficient_data").length,
      models,
    };
    const outputDirectory = resolve("forecast-artifacts");
    await mkdir(outputDirectory, { recursive: true });
    await writeFile(resolve(outputDirectory, "forecast.json"), JSON.stringify(result, null, 2) + "\n", { flag: "wx" });
    await writeFile(resolve(outputDirectory, "summary.json"), JSON.stringify(summary, null, 2) + "\n", { flag: "wx" });
    console.log(`[forecast-dry-run] completed in ${summary.durationSeconds}s; successful=${summary.successfulModels}; failed=${summary.failedModels}; insufficient=${summary.insufficientModels}`);
    ...
    if (summary.failedModels > 0 || summary.successfulModels === 0) {
      throw new Error("Forecast validation failed; inspect the downloaded summary artifact.");
    }
  } finally {
    clearTimeout(timer);
...
main().catch(() => {
  ...
  console.error("[forecast-dry-run] Failed. Check the testing secret, dataset ID, database network access, Python setup, and any available summary artifact.");
  process.exitCode = 1;
});
