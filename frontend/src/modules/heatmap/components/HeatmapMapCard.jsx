...
import legislativeDistrictsUrl from "../data/manila-barangays-with-legislative-districts.json?url";
...
export default function HeatmapMapCard({
  ...
  const [legislativeDistricts, setLegislativeDistricts] = useState(null);
  ...
  useEffect(() => {
    ...
    async function loadDistrictGeometry() {
      try {
        const response = await fetch(legislativeDistrictsUrl, {
        ...
        setLegislativeDistricts(geometry);
        setGeometryError("");
      } catch (error) {
        if (error instanceof DOMException && error.name === "AbortError") return;
        setGeometryError("The Manila district boundaries could not be loaded.");
      }
    }
  ...
  return (
    <div className="col-span-12 self-start rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6 lg:col-span-9">
      ...
      <div className="relative h-[55dvh] min-h-96 overflow-hidden rounded sm:h-[36rem] lg:h-[644px]">
        ...
        {!legislativeDistricts && !geometryError && (
          <div className="absolute inset-x-3 top-3 z-[999] rounded-lg border border-gray-200 bg-white/95 px-3 py-2 text-sm text-gray-700 shadow-sm">
            Loading Manila district boundaries…
          </div>
        )}

        {geometryError && (
          <div role="alert" className="absolute inset-x-3 top-3 z-[999] rounded-lg border border-red-200 bg-red-50/95 px-3 py-2 text-sm text-red-700 shadow-sm">
            {geometryError}
          </div>
        )}
