...
export default function ReportWorkflowPanel({ report, token, onUpdated }) {
  ...
  const [investigation, setInvestigation] = useState({
    investigationDate: report.investigation?.investigationDate
      ? new Date(report.investigation.investigationDate)
          .toISOString()
          .slice(0, 10)
      : new Date().toISOString().slice(0, 10),
    locationVisited:
      report.investigation?.locationVisited || report.location?.name || "",
    ...
    symptoms: Array.isArray(report.investigation?.symptoms)
      ...
      : Array.isArray(report.symptoms)
        ...
        : "",
    foodExposureInformation:
      report.investigation?.foodExposureInformation || report.foodSource || "",
  ...
  const submit = async (action, path, body) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/reports/${report._id}/${path}`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok)
        throw new Error(data.message || "Unable to update this report.");
      setMessage(data.message || "Report updated.");
      await onUpdated?.();
    } catch (requestError) {
      setError(
        getErrorMessage(requestError, "The report could not be updated."),
      );
    } finally {
    ...
    try {
      ...
      const result =
        decision === "suspected"
          ...
          : await postAction("rule-out", {
              reason: ruleOutReason,
              remarks: suspectedRemarks,
            });
      ...
      setError(
        getErrorMessage(
          requestError,
          "The investigation decision could not be recorded.",
        ),
      );
      await onUpdated?.();
    } finally {
  ...
  const investigatedDisease =
    report.disease || report.investigation?.suspectedDisease || "";
  const probableEvidence =
    DISEASE_CASE_RULES[investigatedDisease]?.probableEvidence || [];
  const validationLabel = ["ruled_out", "not_suspected"].includes(status)
    ? "not_applicable"
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-4">
        ...
        <p className="mt-1 text-sm text-blue-900/75">
          Marking a Reported case as Suspected concludes its investigation. A
          Suspected case may become Probable where the disease criteria allow
          it; Probable cases still require confirmation.
        </p>
        <div className="mt-3 flex flex-wrap gap-2 text-xs font-medium">
          <span className="rounded-full bg-blue-50 px-2.5 py-1 text-blue-700">
            Investigation:{" "}
            {formatStatusLabel(report.investigationStatus || "not_started")}
          </span>
          <span className="rounded-full bg-blue-50 px-2.5 py-1 text-blue-700">
            Confirmation: {formatStatusLabel(validationLabel)}
          </span>
        </div>
      </div>
      ...
      {report.investigationStatus === "completed" && report.investigation && (
        <details
          open
          className="group overflow-hidden rounded-xl border border-amber-200 bg-white text-sm text-gray-700"
        >
          ...
          <dl className="grid grid-cols-1 gap-3 border-t border-amber-100 px-4 py-4 md:grid-cols-2">
            <div>
              ...
              <dd>
                {report.investigation.investigationDate
                  ? new Date(
                      report.investigation.investigationDate,
                    ).toLocaleDateString()
                  : "—"}
            ...
            <div>
              <dt className="font-medium">Location visited</dt>
              <dd>{report.investigation.locationVisited || "—"}</dd>
            </div>
            <div>
              <dt className="font-medium">Suspected disease</dt>
              <dd>
                {report.investigation.suspectedDisease || report.disease || "—"}
              </dd>
            </div>
            <div className="md:col-span-2">
              <dt className="font-medium">Findings</dt>
              <dd>{report.investigation.findings || "—"}</dd>
            </div>
            <div>
              <dt className="font-medium">Food/exposure</dt>
              <dd>{report.investigation.foodExposureInformation || "—"}</dd>
            </div>
            <div>
              <dt className="font-medium">Remarks</dt>
              <dd>{report.investigation.remarks || "—"}</dd>
            </div>
          </dl>
        </details>
      )}
      ...
      {report.validation?.validatedAt && (
        <details
          open
          className={`group overflow-hidden rounded-xl border bg-white text-sm text-gray-700 ${report.validation.result === "confirmed" ? "border-red-200" : "border-emerald-200"}`}
        >
          <summary
            className={`flex min-h-12 cursor-pointer list-none items-center justify-between px-4 py-3 font-semibold marker:content-none ${report.validation.result === "confirmed" ? "bg-red-50 text-red-950 hover:bg-red-100/70" : "bg-emerald-50 text-emerald-950 hover:bg-emerald-100/70"}`}
          >
            Confirmation record
            <ChevronDown
              className={`h-4 w-4 transition group-open:rotate-180 ${report.validation.result === "confirmed" ? "text-red-700" : "text-emerald-700"}`}
            />
          </summary>
          <div
            className={`space-y-1 border-t px-4 py-4 ${report.validation.result === "confirmed" ? "border-red-100" : "border-emerald-100"}`}
          >
            <p>
              Recorded{" "}
              {new Date(report.validation.validatedAt).toLocaleString()} by{" "}
              {report.validation.validatedBy?.username ||
                "authorized personnel"}
              .
            </p>
            <p>
              <span className="font-medium">Result:</span>{" "}
              {formatStatusLabel(report.validation.result)}
            </p>
            <p>
              <span className="font-medium">Supporting findings:</span>{" "}
              {report.validation.supportingFindings || "—"}
            </p>
            <p>
              <span className="font-medium">Laboratory evidence:</span>{" "}
              {report.validation.laboratoryEvidence || "—"}
            </p>
            <p>
        ...
        >
          <div>
            <h5 className="font-semibold text-gray-900">
              Investigate and classify report
            </h5>
            <p className="mt-1 text-sm text-gray-600">
              Record the visit findings, then conclude the investigation by
              marking the report as Suspected or ruling it out.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <Field label="Location visited">
              <input
                required
                className={inputClass}
                value={investigation.locationVisited}
                onChange={(e) =>
                  setInvestigation((v) => ({
                    ...v,
          ...
          <Field label="Suspected surveillance disease">
            <select
              required
              className={inputClass}
              value={investigation.suspectedDisease}
              onChange={(e) =>
                setInvestigation((v) => ({
                  ...v,
          ...
          <Field label="Investigation findings">
            <textarea
              ...
              onChange={(e) =>
                setInvestigation((v) => ({ ...v, findings: e.target.value }))
              }
            />
          </Field>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <Field label="Relevant symptoms (comma-separated)">
              <input
                className={inputClass}
                value={investigation.symptoms}
                onChange={(e) =>
                  setInvestigation((v) => ({ ...v, symptoms: e.target.value }))
                }
              />
            </Field>
            <Field label="Food/exposure information">
              <input
                className={inputClass}
                value={investigation.foodExposureInformation}
                onChange={(e) =>
                  setInvestigation((v) => ({
                    ...v,
                    foodExposureInformation: e.target.value,
                  }))
                }
              />
            </Field>
          ...
          <Field label="Additional remarks">
            <textarea
              ...
              onChange={(e) =>
                setInvestigation((v) => ({ ...v, remarks: e.target.value }))
              }
            />
          </Field>
          <div className="grid gap-3 border-t border-blue-100 pt-3 md:grid-cols-2">
            <Field label="Decision remarks">
              <textarea
                rows={2}
                className={inputClass}
                value={suspectedRemarks}
                onChange={(e) => setSuspectedRemarks(e.target.value)}
              />
            </Field>
            <Field label="Reason for ruling out">
              <select
                className={inputClass}
                value={ruleOutReason}
                onChange={(e) => setRuleOutReason(e.target.value)}
              >
                <option value="">Select only when ruling out</option>
                <option value="fake_report">Fake report</option>
                <option value="not_foodborne_related">
                  Not foodborne-related
                </option>
      ...
      {canMarkSuspected && (
        <div className="space-y-3 rounded-xl border border-amber-200 bg-amber-50/40 p-4">
          <h5 className="font-semibold text-gray-900">
            Complete the investigation decision
          </h5>
          <Field label="Decision remarks">
            <textarea
              rows={2}
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <button
              ...
              disabled={Boolean(busyAction)}
              onClick={() =>
                submit("suspected", "mark-suspected", {
                  remarks: suspectedRemarks,
                })
              }
              className="min-h-11 rounded-lg bg-amber-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-amber-700 disabled:opacity-50"
      ...
      {canValidate && (
        <form
          ...
          onSubmit={(event) => {
            event.preventDefault();
            submit("validation", "validation", validation);
          }}
        >
          <h5 className="font-semibold text-gray-900">
            {status === "probable"
              ? "Complete probable-case confirmation"
              : "Classify suspected case"}
          </h5>
          <p className="text-sm text-gray-600">
            Investigated disease: <strong>{investigatedDisease}</strong>
          </p>
          <Field label="Classification result">
            ...
            >
              {status === "suspected" && probableEvidence.length > 0 && (
          ...
          <Field label="Supporting findings">
            <textarea
              ...
              rows={3}
              className={inputClass}
              value={validation.supportingFindings}
              onChange={(e) =>
                setValidation((v) => ({
                  ...v,
                  supportingFindings: e.target.value,
                }))
              }
            />
          </Field>
            ...
            <>
              <Field label="Required probable-case evidence">
                ...
                >
                  ...
                  {probableEvidence.map((evidence) => (
                    <option key={evidence.value} value={evidence.value}>
                      {evidence.label}
                    </option>
                  ))}
                </select>
              ...
              <Field label="Evidence details">
                <textarea
                  ...
                  rows={2}
                  className={inputClass}
                  value={validation.evidenceDetails}
                  onChange={(e) =>
                    setValidation((v) => ({
                      ...v,
            ...
            <>
              <Field label="Laboratory evidence (if applicable)">
                <textarea
                  rows={2}
                  className={inputClass}
                  value={validation.laboratoryEvidence}
                  onChange={(e) =>
                    setValidation((v) => ({
                      ...v,
                      laboratoryEvidence: e.target.value,
                    }))
                  }
                  placeholder="Record the laboratory test/result reference when available."
                />
              </Field>
            </>
          )}
          <Field label="Remarks">
            <textarea
              rows={2}
              className={inputClass}
              value={validation.remarks}
              onChange={(e) =>
                setValidation((v) => ({ ...v, remarks: e.target.value }))
              }
            />
          </Field>
          <button
